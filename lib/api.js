...
module.exports = Class.create({
	...
	loadSession: function (args, callback) {
		...
		if (wfsig && wfid) {
			...
			let wfJob = self.findJob(wfid);
			if(wfJob && wfsig == self.workflowKeys.get(wfid) ) {
				// Workflow subrequests must not escape the category and target of
				// the workflow event which launched them.
				let privileges = { run_events: 1, abort_events: 1, cat_limit: 1, grp_limit: 1 };
				privileges['cat_' + wfJob.category] = 1;
				privileges['grp_' + wfJob.target] = 1;
				return callback(null
				, { type: "workflow", workflow: true, username: "internal" }
				, { username: "internal", workflow: wfid, signature: wfsig, active: true, privileges: privileges }
				);
			}
			else {
