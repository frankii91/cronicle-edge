...
module.exports = Class.create({
	...
	loadSession: function (args, callback) {
		...
		if (wfsig && wfid) {
			let workflowAuth = self.workflowKeys.get(wfid)
			if (!workflowAuth || (wfsig != workflowAuth.signature)) {
				return callback(new Error(`Invalid WorkFlow Signature`), null, null);
			}

			let finishWorkflowAuth = function(err, user) {
				if (err || !user) return callback(new Error(`Workflow owner is invalid or no longer exists`), null, null)
				let session = { type: "workflow", workflow: true, username: user.username || "internal" }
				user.workflow = wfid
				user.signature = wfsig
				callback(null, session, user)
			}

			// Child events must inherit the privileges of the user or API key that
			// launched the workflow, rather than an unrestricted internal account.
			if (workflowAuth.api_key) {
				return self.storage.listFind('global/api_keys', { key: workflowAuth.api_key }, finishWorkflowAuth)
			}
			if (workflowAuth.username) {
				return self.storage.get('users/' + self.usermgr.normalizeUsername(workflowAuth.username), finishWorkflowAuth)
			}
			return callback(new Error(`Workflow has no authorization owner`), null, null)
		}
