...
module.exports = Class.create({
	...
	loadSession: function (args, callback) {
		...
		if (wfsig && wfid) {
			let workflowKey = self.workflowKeys.get(wfid)
			if(workflowKey && wfsig === workflowKey.signature) {
				let privileges = { run_events: 1, abort_events: 1, cat_limit: 1, grp_limit: 1 }
				privileges['cat_' + workflowKey.category] = 1
				privileges['grp_' + workflowKey.target] = 1
