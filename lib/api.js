...
module.exports = Class.create({
	...
	requireValidEventData: function (event, callback) {
		...
		// Workflow data is persisted and rendered by the schedule editor, so validate
		// every nested item rather than relying on the top-level optional rules.
		if ("workflow" in event) {
			if (!Tools.isaArray(event.workflow)) {
				this.doError('api', "Malformed event parameter: workflow (must be array)", callback);
				return false;
			}
			for (var wf_idx = 0; wf_idx < event.workflow.length; wf_idx++) {
				var wf_item = event.workflow[wf_idx];
				if (!wf_item || (typeof (wf_item) != 'object') || Tools.isaArray(wf_item) ||
					(typeof (wf_item.id) != 'string') || !wf_item.id.match(RE_ALPHANUM) ||
					(('arg' in wf_item) && (typeof (wf_item.arg) != 'string')) ||
					(('title' in wf_item) && ((typeof (wf_item.title) != 'string') || !!wf_item.title.match(/[<>]/))) ||
					(('wait' in wf_item) && (typeof (wf_item.wait) != 'boolean')) ||
					(('disabled' in wf_item) && (typeof (wf_item.disabled) != 'boolean'))) {
					this.doError('api', "Malformed workflow item at index: " + wf_idx, callback);
					return false;
				}
			}
		}
