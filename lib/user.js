...
module.exports = Class.create({
	...
	api_callback: async function (args, callback) {
		...
		try {
			...
			// The storage key is lossy (e.g. "ad-min" and "admin" collide), so
			// require the provider identity to match the stored account as well.
			if (!user || user.username.toString().toLowerCase() !== userName.toString().toLowerCase()) {
