...
module.exports = {
	...
	tests: [
		...
		function testAccessSessionRecoveryNavigation(test) {
			var pageDefinition = null;
			var errorCallback = null;
			var assignedLocation = '';
			var sandboxApp = {
				_accessSessionRecoveryRedirect: false,
				api: {
					post: function(endpoint, params, success, error) {
						test.ok(endpoint == 'user/resume_session', 'Session recovery endpoint is used');
						errorCallback = error;
					}
				},
			...
			var sandbox = {
				Class: { subclass: function(parent, name, definition) { pageDefinition = definition; } },
				Page: {},
				app: sandboxApp,
				...
				window: {
					location: {
						...
						assign: function(value) { assignedLocation = value; }
					}
				},
			...
			var source = fs.readFileSync(path.join(__dirname, '../htdocs/js/pages/Base.class.js'), 'utf8');
			vm.runInNewContext(source, sandbox);

			var result = pageDefinition.requireLogin.call({ ID: 'Home', div: { hide: function() {} } });
			test.ok(result === false, 'Protected page waits for session recovery');
			test.ok(typeof(errorCallback) == 'function', 'Session recovery error callback is registered');
			errorCallback();
			test.ok(assignedLocation == 'https://cron.example/#Home', 'API rejection retries as top-level navigation');
			assignedLocation = '';
			errorCallback();
			test.ok(assignedLocation === '', 'Only one top-level retry is triggered');
			test.done();
		},
