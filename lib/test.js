...
module.exports = {
	...
	tests: [
		...
		function testWorkflowSessionPrivileges(test) {
			var workflowId = 'workflowtest';
			var signature = 'workflow-signature';
			cronicle.workflowKeys.set(workflowId, {
				signature: signature,
				category: 'general',
				target: 'allgrp'
			});
			...
			cronicle.loadSession({
				...
				params: {},
				query: {},
				request: { headers: { 'x-wf-id': workflowId, 'x-wf-signature': signature } }
			}, function(err, session, user) {
				cronicle.workflowKeys.delete(workflowId);
				test.ok(!err, 'Workflow signature was accepted');
				test.ok(session && session.workflow, 'Workflow session was created');
				test.ok(user.privileges.cat_limit, 'Workflow session has category limits');
				test.ok(user.privileges.grp_limit, 'Workflow session has group limits');
				test.ok(user.privileges.cat_general, 'Parent category is allowed');
				test.ok(user.privileges.grp_allgrp, 'Parent target is allowed');
				test.ok(!user.privileges.cat_restricted, 'Unrelated category is not allowed');
				test.done();
			});
		},
