...
module.exports = {
	...
	tests: [
		...
		function testFetchJobLogPathConfinement(test) {
			var outside_log = path.resolve(config.log_dir, 'outside.log');
			fs.mkdirSync(path.dirname(outside_log), { recursive: true });
			fs.writeFileSync(outside_log, 'sensitive');

			cronicle.api_fetch_delete_job_log({
				query: {
					path: outside_log,
					auth: Tools.digestHex(outside_log + config.secret_key)
				}
			}, function(status, headers, body) {
				test.ok( status == '403 Forbidden', "Log fetch rejects a signed path outside the job log directory" );
				test.ok( fs.existsSync(outside_log), "Rejected log file was not deleted" );
				fs.unlinkSync(outside_log);
				test.done();
			});
		},
