...
if(!process.env['CRONICLE_secret_key'] && !process.env['CRONICLE_secret_key_file'] && !fs.existsSync(secret_key_file)) {
	// Generate the default cluster secret at first startup, never while building a distributable bundle.
	fs.mkdirSync(require('path').dirname(secret_key_file), { recursive: true, mode: 0o700 });
	try {
		fs.writeFileSync(secret_key_file, crypto.randomBytes(32).toString('hex'), { flag: 'wx', mode: 0o400 });
	}
	catch (err) {
		// Another process may have created the shared secret between the existence check and write.
