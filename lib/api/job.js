...
function sanitizeJob(job) {
	// Work on a copy so retries can continue using the private launch context.
	job = Tools.copyHash(job, true);
	delete job.env;
	delete job.secret;
		...
		this.loadSession(args, function (err, session, user) {
			...
			self.storage.head('jobs/' + params.id + '/log.txt.gz', function (err, info) {
				...
				self.storage.get('jobs/' + params.id, function (err, job) {
					...
					callback({ code: 0, job: sanitizeJob(job) });
				}); // job get
			}); // log head
		}); // session
	},
	...
	api_get_job_status: function (args, callback) {
		...
		this.loadSession(args, function (err, session, user) {
			...
			if (job) {
				...
				return callback({
					code: 0,
					job: Tools.mergeHashes(sanitizeJob(job), {
	...
	api_get_active_jobs: function (args, callback) {
		...
		this.loadSession(args, function (err, session, user) {
			...
			for (let id in activeJobs) {
				activeJobs[id] = sanitizeJob(activeJobs[id]);
