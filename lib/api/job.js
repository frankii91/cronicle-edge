...
module.exports = Class.create({
	...
	api_fetch_delete_job_log: function (args, callback) {
		...
		var log_file = path.resolve(query.path);
		var job_log_dir = path.resolve(this.server.config.get('log_dir'), 'jobs');
		var relative_log_file = path.relative(job_log_dir, log_file);

		// Only serve files from the configured job log directory.  In particular,
		// reject absolute paths on both POSIX and Windows, even when correctly signed.
		if (!relative_log_file || relative_log_file.startsWith('..' + path.sep) ||
			path.isAbsolute(relative_log_file)) {
