	...
	api_run_event: function (args, callback) {
		...
		this.loadSession(args, function (err, session, user) {
			...
			self.storage.listFind('global/schedule', criteria, function (err, event) {
				...
				delete params.uid;
				delete params.secret;
				// chain data is internal scheduler state and must not be supplied by callers
				...
				// only allow event customization (like shell scripts and memos) to users with edit priv
