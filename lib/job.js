...
module.exports = Class.create({
	...
	launchLocalJob: function (job) {
		...
		// Build the configured environment only on the executing worker.  Keeping it
		// off the job object prevents credentials from leaking through job metadata.
		let job_env = Tools.mergeHashes(process.env, self.server.config.get('job_env') || {})
		let web_server = self.server.config.get('WebServer')
		let proto = web_server.https ? 'https://' : 'http://'
		let port = web_server.https ? web_server.https_port : web_server.http_port
		let base_url = proto + self.server.hostname + ':' + port
		let base_path = String(self.server.config.get('base_path') || '').trim()
		if ((/^\/\w+$/i).test(base_path)) base_url += base_path
		job_env['BASE_URL'] = base_url
		job_env['BASE_APP_URL'] = self.server.config.get('base_app_url') || ''
